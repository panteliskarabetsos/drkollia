...
import { CheckCircle, CalendarCheck2, Clock, FileText, MailCheck } from 'lucide-react';
...
export default function AppointmentSuccessClient() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-[#f9f9f9] px-4 py-24">
      ...
      >
        <CheckCircle className="mx-auto text-green-600 mb-4" size={48} strokeWidth={1.5} />
        <h1 className="text-2xl font-semibold text-gray-800 mb-1">Ευχαριστούμε, {name}!</h1>
        <p className="text-gray-600 mb-6">Το ραντεβού σας επιβεβαιώθηκε με επιτυχία.</p>
        ...
        <div className="bg-gray-100 rounded-xl p-5 text-left text-sm text-gray-800 space-y-3 mb-6">
          <div className="flex items-center gap-2">
            ...
            <span><strong>Ημερομηνία:</strong> {formattedDate}</span>
          </div>
          <div className="flex items-center gap-2">
            <Clock className="text-gray-600" size={18} />
            <span><strong>Ώρα:</strong> {formattedTime}</span>
          </div>
          <div className="flex items-center gap-2">
            <FileText className="text-gray-600" size={18} />
            <span><strong>Λόγος:</strong> {reason}</span>
          </div>
        </div>
        ...
        <p className="text-sm text-gray-400 mt-4">
          Αυτόματη επιστροφή σε 15 δευτερόλεπτα...
        </p>
      </motion.div>
    </main>
  );
